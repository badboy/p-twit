...
static void __triHeapFindLargestBlock( triHeap* mem )
{
	...
	{
		triS32 csize = __CBLOCK_GET_SIZE(mblocks[i]);
...
void* triHeapRealloc( triHeap* mem, void* ptr, triU32 size )
{
	...
	if (bsize>csize+mem->free) return(0);	// User wants more than is possible
	...
	if (bsize<csize)
	{
		...
		if (mem->_largestblock < newsize)
		{
			mem->_largestblock = newsize;
