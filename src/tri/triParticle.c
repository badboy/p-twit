...
static inline void triParticleCreate( triParticleEmitter* e, triParticle* p )
{
	...
	p->lifetime = e->life + randVec.y*e->life*e->lifeRand;	// lifetime +- 20%
	p->age = 0.0f;
	p->rand = randVec.z;
...
static inline void triVortexCreate( triParticleEmitter* e, triVortex* v )
{
	...
	v->lifetime = e->life*1.5 + randVec.x*e->life*0.2f;	// lifetime +- 20%
	v->age = 0.0f;
	
	v->confinement = e->vortexRange + randVec.y*e->vortexRange*0.2f;
	v->dir = randVec.y<0?(-0.6f + randVec.y*0.4f):(0.6f + randVec.y*0.4f);
